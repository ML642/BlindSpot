...
export async function createSampleAudit(store: ArtifactStore): Promise<Audit> {
  ...
  try {
    ...
    const state: PageState = { id: randomUUID(), url: 'https://sample.blindspot.example/sign-in', title: 'Fieldwork sign-in sample', capturedAt: now, description: 'Illustrative sign-in page, rendered locally', screenshotArtifactId: screenshot.id, domArtifactId: dom.id, accessibilityArtifactId: aria.id };
    const ids = ['blindness', 'low-vision', 'motor', 'cognitive'] as const;
    const axe = await new AxeBuilder({ page }).analyze();
    const result = await runAutomatedChecks({ page, pageState: state, axeResults: { violations: axe.violations.map(rule => ({ ...rule, nodes: rule.nodes.map(node => ({ ...node, target: node.target.map(String) })) })) } }, ids);
