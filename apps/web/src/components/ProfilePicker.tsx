...
export function ProfilePicker({ selected, onChange, single = false }: { selected: ProfileId[]; onChange: (next: ProfileId[]) => void; single?: boolean }) {
  const allSelected = selected.length === profiles.length;
  const toggle = (id: ProfileId) => onChange(single ? [id] : selected.includes(id) ? selected.filter((value) => value !== id) : [...selected, id]);
  ...
  return (
    <fieldset className="profile-fieldset">
      ...
      <div className="profile-heading">
        ...
        {!single && <button type="button" className="select-all" onClick={toggleAll} aria-pressed={allSelected}>{allSelected ? 'Clear all' : 'Select all'}</button>}
      </div>
          ...
          {column.groups.map((group) => <div className="profile-group" key={group}><h3>{group}</h3><div className="profile-options">
            {profiles.filter((profile) => profile.group === group).map((profile) => {
              ...
              return <label className={`profile-option ${isSelected ? 'is-selected' : ''}`} key={profile.id}>
                <input type={single ? 'radio' : 'checkbox'} name={single ? 'demo-profile' : undefined} checked={isSelected} onChange={() => toggle(profile.id)} />
      ...
      <p className={`selection-count ${selected.length === 0 ? 'is-error' : ''}`} aria-live="polite">{single ? 'Free demo: choose one profile per audit.' : `${selected.length} of ${profiles.length} profiles selected`}</p>
    </fieldset>
  );
}
